...
public class QEventWrapperQueuer implements Quittable {
	 ...
	 * @param ringBuffer
	 */
	public QEventWrapperQueuer(Disruptor<QEventWrapper> disruptor,
			RingBuffer<QEventWrapper> ringBuffer) {
	 ...
	 */
	public QEventWrapperQueuer(Disruptor<QEventWrapper> disruptor,
			RingBuffer<QEventWrapper> ringBuffer, String logFileName) {
		if (disruptor == null) {
		...
		if (logFileName != null) {
			...
			try {
				...
				logWriter = Files.newBufferedWriter(newFile,
						Charset.defaultCharset());
			} catch (IOException e) {
				getLog().error(
						"Error creating event Log File, " + logFileName + "\n"
								+ e);
			}
		}
	...
	private static final EventTranslatorOneArg<QEventWrapper, QEventWrapper> TRANSLATOR = new EventTranslatorOneArg<QEventWrapper, QEventWrapper>() {
		public void translateTo(QEventWrapper event, long sequence,
				QEventWrapper incoming) {
			event.set(incoming);
		}
	};
	...
	public void onData(List<QEventWrapper> incomingList) {
		for (QEventWrapper qew : incomingList) {
			onData(qew);
		}
	}
