...
public class QEventWrapper {
	...
	public void resetEvent() {
		...
		case CHECK_VERSION:
			this.setEvent(new QEventCheckVersion((String) null, (String) null));
			break;
		case INITIATE_SESSION:
			this.setEvent(new QEventInitiateSession((String) null,
					(String) null));
			break;
		case VOID:
			this.setEvent(new QEventVoid());
			break;
		default:
			this.setEvent(null);
			break;
	...
	public void resetEventHandler() {
		switch (this.getEventType()) {
		case CHECK_VERSION:
			this.setHandler(new QEventHandlerCheckVersion());
			break;
		case INITIATE_SESSION:
	...
	public void checkConsistency() {
		...
		switch (this.getEventType()) {
		case CHECK_VERSION:
			problem = (!(this.getEvent() instanceof QEventCheckVersion));
			problem |= (!(this.getHandler() instanceof QEventHandlerCheckVersion));
			break;
		case INITIATE_SESSION:
		...
		if (problem) {
			throw new IllegalArgumentException("EventType:"
					+ this.getEventType() + " is inconsistent with Event:"
					+ this.getEvent().getClass().getCanonicalName());
		}
	}
	...
	public QEventWrapper(QEventType eventType, QEvent event,
			QEventHandlerResultListener resultListener) {
		this(System.currentTimeMillis(), eventType, event, resultListener);
	}

	QEventWrapper(long eventTime, QEventType eventType, QEvent event,
			QEventHandlerResultListener resultListener) {
	...
	public QEventWrapper(QEventType eventType, QEvent event,
			List<QEventHandlerResultListener> resultListeners) {
	...
	static public QEventWrapper fromJSON(JSONObject in) {
		...
		QEventType eventType = QEventType.fromString((String) in
				.get("eventType"));
		QEvent event;
		switch (eventType) {
		case CHECK_VERSION:
		...
		case VOID:
			...
			break;
		default:
			event = QEvent.fromJSON((JSONObject) in.get("event"));
			break;
		}
		QEventWrapper ret = new QEventWrapper(eventTime, eventType, event,
