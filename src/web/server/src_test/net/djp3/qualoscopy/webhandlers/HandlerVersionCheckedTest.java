...
public class HandlerVersionCheckedTest {
	...
	public void testWebServer() {
		...
		/* Check version with version required, but without providing it. */
		responseString = null;
		try {
		...
		/* Check version with version required, and with providing it, but with null. */
		responseString = null;
		try {
			URIBuilder uriBuilder = new URIBuilder().setScheme("http")
					.setHost("localhost")
		...
		/* Check version with version required, and with providing it, but the wrong one. */
		responseString = null;
		try {
			URIBuilder uriBuilder = new URIBuilder().setScheme("http")
					.setHost("localhost")
					.setPort(ws.getInputChannel().getPort())
					.setPath("/version")
					.setParameter("version", Globals.getGlobals().getSystemVersion()+"foo");
		...
		try {
			...
			JSONArray errors = (JSONArray) response.get("errors");
			String error = (String) errors.get(0);
			assertTrue(error.contains(QEventHandlerCheckVersion.ERROR_API_VERSION_MISMATCH));
		} catch (ClassCastException e) {
			fail("Bad JSON Response");
		}
