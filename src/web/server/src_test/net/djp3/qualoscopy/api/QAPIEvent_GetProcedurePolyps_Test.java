...
public class QAPIEvent_GetProcedurePolyps_Test {
	...
	public void test() {
		...
		try{
			...
			QAPIEvent_GetProcedurePolyps a = new QAPIEvent_GetProcedurePolyps(version,null);
			QAPIEvent_GetProcedurePolyps b = (QAPIEvent_GetProcedurePolyps) a.clone();
	...
	public void testWebServerSocket() {
		...
		/**** Now test get polyp without procedure id ****/
		responseString = null;
		try {
			URIBuilder uriBuilder = new URIBuilder()
									.setScheme("http")
		...
		/**** Now test get polyp with bad credentials ****/
		responseString = null;
		try {
			URIBuilder uriBuilder = new URIBuilder()
									.setScheme("http")
		...
		try {
			...
			for(int i = 0;i < polyps.size();i++){
				...
				assertTrue(polyp.size() == 2);
				assertTrue(polyp.get("polyp_id") != null);
				assertTrue(polyp.get("time_removed") != null);
			}
